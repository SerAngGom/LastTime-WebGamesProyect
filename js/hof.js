...
class HallOfFame {
    ...
    displayOnStage(x, y, w, h) {
        ...
        let hL = Math.floor(0.08 * h);
        let sL = Math.floor(0.3 * 0.1 * 0.8 * h);
        let wCP = Math.floor(0.06 * w);
        let wCS = Math.floor(0.18 * w);
        let margenFecha = Math.floor(0.15 * w);
        let wCD = Math.floor(0.61 * w);
...
function createHOF() {
    ...
    shooterHOF.displayOnStage(0, 20, gameWidth, gameHeight * 0.7);

    let msg = game.add.text(0, 0, msgUser,
              {font:'bold 18px Arial', fill:'#00FFFF',
               boundsAlignH:'center', boundsAlignV:'middle',
                wordWrap: true, wordWrapWidth: gameWidth - 20});
    msg.setTextBounds(0, game.world.height-140, game.world.width, 60);

    let again = game.add.text(0, 0, "Do you want to shoot again? Press 'R' to restart.",
                {font:'bold 20px Arial', fill:'#FFFFFF',
                 boundsAlignH:'center', boundsAlignV:'middle'});
    again.setTextBounds(0, game.world.height-60, game.world.width, 40);
